...
- 比较新老 `children` 的 `length` 获取最小值 然后对于相同部分，进行从新patch工作。
...
这一步的事情就是从头开始比较新对节点，如果相同的话执行 `patch`，如果发现有不相同的节点，立即跳出循环
...
**三-1、如果旧节点都遍历完了，旧的节点还没有遍历完，走下面的分支：**
...
遍历剩余的新节点，如果节点存在 `key`,以 `key` 为键保存到 `keyToNewIndexMap` 中，如果存在相同的 `key`的节点的，则提示 `warn`
