...
- 每个对象的属性再通上一步中的 `depsMap` 保存着对应的 `dep`， `depsMap 是一个 `Map` 结构
...
- 先从 `data` 对象中访问 `counter` 的属性
 
   - 对于访问的对象 ` counter: { cou: 1 }` 生成对应的 `depsMap`
   
   - 对于访问的属性 `counter` 生成对应在的 `dep`，添加收集当前 `effect`
   
- 之后访问 `counter` 对象中访问 `cou` 的属性
 
   - 对于访问的对象 `{ cou: 1 }` 生成对应的 `depsMap`
   
   - 对于访问的属性 `cou` 生成对应在的 `dep`，添加收集当前 `effect`
   
通过 `proxy`，不用遍历子属性，也可以做到响应监听
...
function set(target, key, value, receiver) {
  ...
  const result = Reflect.set(target, key, value, receiver);
  if (target === toRaw(receiver)) {
    // hadKey 为 false，所以执行 trigger 方法收集依赖
    if (!hadKey) {
      trigger(target, "add" /* ADD */, key, value);
    }
    else if (hasChanged(value, oldValue)) {
...
2. 调用`trigger(target, "add" /* ADD */, key, value);`
