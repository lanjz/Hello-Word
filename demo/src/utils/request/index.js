...
const requestHandler = (config = {}) => {
	config.headers['x-sf-custom-tenantcode'] = store.state.user.tenantCode;
	config.headers['x-sf-custom-sharecentercode'] = store.state.user.shareCenterCode;
	config.headers['x-sf-username'] = store.state.user.loginInfo.userName;
	if (process.env.NODE_ENV === 'development') {
		config.headers['x-sf-custom-tenantcode'] = config.headers['x-sf-custom-tenantcode'] || '5';
		config.headers['x-sf-custom-sharecentercode'] = config.headers['x-sf-custom-sharecentercode'] || 'FSC-SFSZ';
		config.headers['x-sf-username'] = config.headers['x-sf-username'] || '01419172';
	}
