		...
		<div class="left global-shrink global-flex-column">
          ...
          <template #default="{ node, data }">
            ...
            >
              ...
              <span>
							<i
                  class="icon-shanchu iconfont global-warning"
                  style="font-size: 15px; padding: 0 5px"
                  type="text"
                  @click.stop="deleteItem(node, data)"
              />
							<i
                  class="icon-Add iconfont global-primary"
                  style="font-size: 15px; padding: 0 5px"
                  type="text"
		...
		<div class="right global-flex-1 scroll">
			...
			<template v-else>
				...
				<div class="border-box right">
					<div class="parent-info" v-if="isAdd">
						<span >所属父级：{{ moduleForm.parentName || systemName }}</span>
					</div>
					<el-form
						:rules="rules"
						label-position="left"
...
export default {
	...
	computed: {
		systemName() {
			return (this.treeData[0] && this.treeData[0].moduleName) || 'Root';
		},
	},
	mounted() {
